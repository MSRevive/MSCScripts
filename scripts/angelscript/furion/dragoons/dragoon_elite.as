...
class DragoonElite : CGameScript
{
	...
	{
		if (!(WEAPON == 2)) return;
		SetName("Elite Dragoon Martial Artist");
		SetHealth(1000);
		SetMoveSpeed(1.5);
	...
	{
		if (!(WEAPON == 3)) return;
		SetName("Elite Dragoon Swordsman");
		SetHealth(1000);
		MOVE_RANGE = 60;
		ATTACK_RANGE = 100;
	...
	{
		if (!(WEAPON == 5)) return;
		SetName("Elite Dragoon Berserker");
		SetHealth(1750);
		MOVE_RANGE = 80;
		ATTACK_RANGE = 100;
		ATTACK1_DAMAGE = Random(30, 60);
	...
	void cycle_up()
	{
		...
		if (BANDIT_TYPE == "spear")
		{
			FREQ_SPEC_ATTACK = FREQ_SPEAR;
		}
	...
	void special_attack2()
	{
		if (BANDIT_TYPE == "bow")
		{
			if ((false))
			{
				...
				ScheduleDelayedEvent(0.5, "bow_sound");
				ScheduleDelayedEvent(0.6, "bow_sound");
	...
	void attack()
	{
		...
		if (WEAPON == 0)
		{
			...
			npcatk_adj_attack();
			AS_ATTACKING = GetGameTime();
			TossProjectile("proj_arrow_npc", /* TODO: $relpos */ $relpos(0, 0, 2), "none", ATTACK_SPEED, FINAL_DAMAGE, ATTACK_COF, "none");
	...
	void game_dodamage()
	{
		...
		if (SPECIAL_EFFECT_CHANCE < 30)
		{
			if (BANDIT_TYPE != "bow")
			{
				if (ELEMENT == 0)
				{
					ApplyEffect(param2, "effects/dot_cold", 10.0, GetEntityIndex(GetOwner()), RandomInt(1.0, 5.0));
				}
				if (ELEMENT == 1)
				{
					ApplyEffect(param2, "effects/dot_poison", 4.0, GetEntityIndex(GetOwner()), RandomInt(25.0, 40.0));
				}
				if (ELEMENT == 2)
				{
					ApplyEffect(param2, "effects/dot_fire", 15.0, GetEntityIndex(GetOwner()), RandomInt(5.0, 15.0));
				}
				if (ELEMENT == 3)
				{
					ApplyEffect(param2, "effects/dot_lightning", 2.0, GetEntityIndex(GetOwner()), RandomInt(1.0, 100.0));
				}
			}
		}
	}
