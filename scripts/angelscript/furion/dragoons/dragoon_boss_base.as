...
class DragoonBossBase : CGameScript
{
	int AM_LEAPING;
	string ANIM_ATTACK;
	string ANIM_DEATH;
	...
	DragoonBossBase()
	{
		...
		if (StringToLower(GetMapName()) == "dragooncaves")
		{
			NPC_IS_BOSS = 1;
		}
		...
		const string MONSTER_MODEL = "fur/npcs/dragoonlord.mdl";
	...
	void setup_boss()
	{
		SetName("Demetricus the Dragoon Leader");
		SetHealth(3400);
		SetModelBody(1, 1);
	...
	void OnDamage(int damage) override
	{
		...
		if (!(DID_FIRST_POT))
		{
			...
			DID_FIRST_POT = 1;
			drink_special();
			ScheduleDelayedEvent(90, "end_special_effect");
		}
		if (!(DID_SECOND_POT))
		{
			if (!(DRINKING_POT))
	...
	void drink_done()
	{
		...
		if (DRINK_TYPE == "fire")
		{
			...
			ScheduleDelayedEvent(120, "normal_immune");
			IN_RESIST = 1;
	...
	void bow_shoot()
	{
		...
		if (!(SCATTER_SHOT))
		{
			...
			BALL_DMG = BALL_SIZE;
			BALL_DMG *= 30;
			string OFS_ADJ = /* TODO: $relpos */ $relpos(0, 28, 36);
			if (GetEntityRange(m_hAttackTarget) < 100)
			{
				string OFS_ADJ = /* TODO: $relpos */ $relpos(0, 28, -12);
			}
			TossProjectile("proj_mana", OFS_ADJ, "none", BALL_SPEED, BALL_DMG, 1, "none");
		}
		if ((SCATTER_SHOT))
		{
			EmitSound(GetOwner(), 0, SOUND_SATTACK, 10);
