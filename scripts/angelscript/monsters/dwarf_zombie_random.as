...
class DwarfZombieRandom : CGameScript
{
	...
	DwarfZombieRandom()
	{
		...
		const float ATTACK2_ACCURACY = 0.75;
		ANIM_IDLE = "idle";
		ANIM_WALK = "walk";
		ANIM_RUN = "walk";
		ANIM_DEATH = "death";
		...
		const string ANIM_SLASH = "attack";
		const string ANIM_NOD = "nod";
		...
		const string SOUND_LEAP = "agrunt/ag_attack2.wav";
		...
		const string SOUND_NOD = "x/x_laugh1.wav";
		DROP_GOLD = 1;
		DROP_GOLD_MIN = 5;
		DROP_GOLD_MAX = 30;
		const int I_AM_TURNABLE = 1;
		MONSTER_WIDTH = 32;
		Precache(SOUND_DEATH);
		const Vector3 LANTERN_COLOR = Vector3(LNT_R, LNT_G, LNT_B);
	...
	{
		string MY_POS = GetEntityOrigin(GetOwner());
		string TRAVEL_DIST = Distance(MY_POS, WALKING_MY_OLD_POS);
		if (TRAVEL_DIST > 32)
		{
			if (WALK_STEP == 1)
			{
