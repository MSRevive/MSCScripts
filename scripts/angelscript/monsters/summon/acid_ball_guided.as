...
class AcidBallGuided : CGameScript
{
	string FIRST_TARG;
	int IS_ACTIVE;
	string MY_AOE;
	...
	AcidBallGuided()
	{
		const string SOUND_EXPLODE = "bullchicken/bc_bite3.wav";
	...
	void OnSpawn() override
	{
		...
		SetModelBody(0, 6);
		Effect("glow", GetOwner(), Vector3(0, 255, 0), 16, -1, -1);
	...
	void go_splodie()
	{
		...
		SetModel("none");
		XDoDamage(GetMonsterProperty("origin"), MY_AOE, MY_BASE_DMG, 0.1, MY_OWNER, MY_OWNER, "spellcasting.affliction", "acid");
		MY_DOT = MY_BASE_DMG;
		MY_DOT *= 0.1;
		...
		ClientEvent("new", "all", "effects/sfx_acid_splash", GetEntityOrigin(GetOwner()));
		IS_ACTIVE = 0;
	...
	void push_loop()
	{
		string CUR_TARG = GetToken(PUSH_LIST, i, ";");
		ApplyEffect(CUR_TARG, "effects/dot_poison", 5, MY_OWNER, MY_DOT);
	}
