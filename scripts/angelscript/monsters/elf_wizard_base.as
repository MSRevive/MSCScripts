...
class ElfWizardBase : CGameScript
{
	...
	ElfWizardBase()
	{
		const string ELF_MODEL = "npc/elf_m_wizard.mdl";
		ANIM_IDLE = "idle";
		...
		const string ANIM_MELEE = "ref_shoot_crowbar";
		const string ANIM_PALM_ATTACK = "ref_shoot_trip";
		const string ANIM_XBOW_AIM = "ref_aim_bow";
		const string ANIM_XBOW_FIRE = "ref_shoot_bow";
		const string ANIM_XBOW_RELOAD = "ref_shoot_squeak";
		const string ANIM_DUCK_MOVE = "crawl";
		const string ANIM_MELEE_DUCK = "crouch_shoot_crowbar";
		const string ANIM_XBOW_AIM_DUCK = "crouch_aim_bow";
		const string ANIM_XBOW_FIRE_DUCK = "crouch_shoot_bow";
		const string ANIM_XBOW_RELOAD_DUCK = "crouch_aim_squeak";
		const string ANIM_SPELL_HOLD = "ref_aim_onehanded";
		const string ANIM_SPELL_LEFT_HOLD = "ref_aim_egon";
		const string ANIM_SPELL_PREP = "ref_aim_trip";
		const string ANIM_SPELL_RELEASE = "ref_shoot_trip";
		const string ANIM_REPELL = "ref_shoot_trip";
		const string ANIM_PREP_EGG = "ref_aim_squeak";
		const string ANIM_RELEASE_EGG = "ref_shoot_squeak";
		const string ANIM_SHIELD_HOLD = "aim_2";
		const string ANIM_STAFF_HOLD = "ref_aim_crowbar";
		...
		const int ELF_IS_ARCHER = 0;
		const int ELF_EXPLOSIVE_BOLTS = 1;
		const string ELF_XBOW_CL_SCRIPT = "monsters/elf_xbow_cl";
		const Vector3 ELF_XBOW_STAND_OFS = Vector3(10, 0, 68);
		const Vector3 ELF_XBOW_DUCK_OFS = Vector3(10, 0, 32);
		const int ELF_XBOW_BONE = 27;
		const int DMG_XBOW = 200;
		const float FREQ_XBOW_BASH = 5.0;
		const int ELF_XBOW_ACCURACY = 75;
		const int ATTACK_HITRANGE_MELEE = 64;
		const int ATTACH_STAFF_TIP = 2;
		const int ATTACH_STAFF_HILT = 1;
		const int ATTACH_LHAND = 3;
		const int ATTACH_RHAND = 0;
		const int ELF_IS_NOVICE = 0;
		const float FREQ_GUIDED = 25.0;
		const int ELF_CAN_GUIDED = 0;
		const string ELF_GUIDED_TYPES = "fire;cold;lightning;poison";
		const string ELF_GUIDED_DOTS = "100;75;75;75";
		const int ELF_GUIDED_DMG = 200;
		const float ELF_GUIDED_DURATION = 20.0;
		const string ELF_GUIDED_SCRIPT = "monsters/summon/guided_sphere_cl";
		const int CFB_SPEED = 60;
		const int ELF_PALM_ATTACK = 0;
		const int ELF_PALM_DMG = 300;
		const int ELF_PALM_RANGE = 96;
		const float FREQ_PALM = 3.0;
		ELF_BEAM_DMG = 100;
		ELF_BEAM_DOT = 100;
		ELF_BEAM_DMG_TYPE = "lightning";
		ELF_BEAM_PUSH_VEL = /* TODO: $relvel */ $relvel(0, 300, 110);
		ELF_BEAM_DUR = 5.0;
		const int ELF_BEAM_RANGE = 512;
		ELF_BEAM_SPECIAL = "none";
		const string ELF_PALM_CL_SCRIPT = "effects/sfx_hit_shield";
	...
	void dbolt_dodamage()
	{
		...
		string TARG_DIST = Distance(TARG_ORG, ELF_XBOW_REPELL_POINT);
		TARG_DIST /= 128;
	...
	void setup_guided2()
	{
		...
		N_TYPES -= 1;
		string RND_TYPE = RandomInt(0, N_TYPES);
