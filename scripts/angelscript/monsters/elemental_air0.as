...
class ElementalAir0 : CGameScript
{
	...
	ElementalAir0()
	{
		...
		IS_UNHOLY = 1;
		IMMUNE_VAMPIRE = 1;
		FREQ_ZAP = RandomInt(3, 8);
		FREQ_FLEE = 10.0;
		FREQ_REPOS = Random(1, 3);
		DMG_CHAIN = Random(2, 6);
		BEAM_RANGE = 2048;
		FINGER_ADJ = "$relpos($vec(0,MY_YAW,0),$vec(0,30,0))";
		...
		ATTACK_ACCURACY = 0.8;
		ATTACK_DAMAGE = "$rand(50,100)";
		CIRCLE_RANGE = 256;
		AIM_RATIO = 50;
		ATTACK_CONE_OF_FIRE = 2;
		ATTACK_SPEED = 500;
		FIRE_DAMAGE = "$rand(20,50)";
		AMB_FIRE_DAMAGE = "$rand(10,30)";
		FIRESEAL_DAMAGE = 200;
		FIRE_BALL_DAMAGE = "$rand(50,100)";
	...
	void OnRepeatTimer()
	{
		...
		int RND_FLOAT_X = RandomInt(1, 20);
		int RND_FLOAT_Y = RandomInt(1, 20);
		int RND_FLOAT_Z = RandomInt(1, 20);
	...
	void game_dodamage()
	{
		...
		if ((USE_SWIPE_SOUND))
		{
			int VEL_F = RandomInt(10, 400);
			int VEL_Z = RandomInt(10, 400);
