...
class PetWolfShadow : CGameScript
{
	...
	void game_dodamage()
	{
		if ((MELEE_ATTACK))
		{
			...
			ApplyEffect(param2, "effects/dot_fire", 5.0, GetEntityIndex(GetOwner()), DOT_ICE);
			EmitSound(GetOwner(), 0, "ambience/steamburst1.wav", 10);
			Effect("glow", GetOwner(), Vector3(255, 128, 64), 128, 1, 1);
		}
		MELEE_ATTACK = 0;
	}
	...
	void do_combat_howl()
	{
		...
		ClientEvent("new", "all", "effects/sfx_fire_burst", GetEntityOrigin(GetOwner()), 128, 1, Vector3(128, 128, 255));
	...
	void freeze_targets()
	{
		...
		ApplyEffect(CUR_TARG, "effects/dot_fire", 5.0, GetEntityIndex(GetOwner()), DOT_ICE);
		ApplyEffect(CUR_TARG, "effects/debuff_stun", 5, GetEntityIndex(GetOwner()));
	}
