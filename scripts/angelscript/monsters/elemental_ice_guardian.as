...
class ElementalIceGuardian : CGameScript
{
	...
	{
		const string ICE_GUARD_NAME = "Lesser Ice Guardian";
		const int ICE_GUARD_HP = 3000;
		const float ICE_GUARD_FIRE_VULN = 1.5;
		...
		const string GUARD_ELEMENT = "ice";
		const string ICE_GUARD_DOT_EFFECT = "effects/dot_cold";
		...
		const string ANIM_STUN_BURST = "attack_smash";
		const string ANIM_THROW = "attack_throw";
		const string ANIM_PUSH = "attack_push";
		const string ANIM_LUNGE = "attack_long";
		const string ANIM_ICE_BALL = "release_spell";
		const string ANIM_SPELL_LOOP = "spell_loop";
		const string ANIM_SUMMON = "summon";
		const string ANIM_ALERT = "alert";
		const string ANIM_FLINCH_CUSTOM = "flinch";
		const string ANIM_TAUNT = "taunt";
		const string ANIM_PROJECTILE = "projectile";
		const string ANIM_MULTI_PROJECTILE = "multi_projectile";
		const string ANIM_DODGE = "attack_pushr";
		const string ANIM_PUSHL = "attack_pushl";
		const string ANIM_BREATH_START = "ccastout";
		const string ANIM_BREATH_LOOP = "ccastoutlp";
		const string ANIM_FAKE_DEATH = "long_death";
		const string ANIM_RESSURECT = "ressurect";
		const string ANIM_FAKE_DEATH_IDLE = "dead_idle";
		VEL_PUSH_ATK_F = 400;
		VEL_THROW_ATK_L = -400;
		VEL_THROW_ATK_F = 400;
		VEL_BURST_F = 500;
		...
		const int BFLY_VSPEED_UP = 5;
		const int BFLY_VSPEED_DOWN = -50;
		MOVE_RANGE = 50;
		ATTACK_MOVERANGE = 50;
		const int ATTACK_MOVERANGE_DEF = 50;
		ATTACK_RANGE = 60;
		ATTACK_HITRANGE = 75;
		const int ATTACK_RANGE_DEF = 60;
		const int ATTACK_HITRANGE_DEF = 75;
		const int ATTACK_MOVERANGE_AGRO = 60;
		const int LUNGE_RANGE_MIN = 75;
		const int LUNGE_RANGE_MAX = 175;
		const int LUNGE_RANGE_MAX_HITRANGE = 125;
		const Vector3 STAFF_BEAM_COLOR = Vector3(128, 128, 255);
		const string STAFF_ALT_EFFECT = "effects/dot_lightning";
		NPC_GIVE_EXP = 1000;
		const string FREQ_STUN_BURST = Random(20.0, 30.0);
		const string FREQ_ICE_BALL = Random(20.0, 30.0);
		const string FREQ_ICE_CIRCLE = Random(20.0, 30.0);
		const string FREQ_THROW = Random(10.0, 20.0);
		const float FREQ_STAFF_MODE_CHANGE = 20.0;
		const float FREQ_LUNGE = 5.0;
		const string FREQ_DODGE = Random(8.0, 12.0);
		const float FREQ_FLINCH = 20.0;
		const float FREQ_CHANGE_STANCE = 20.0;
		const float FREQ_TAUNT = 20.0;
		const float FREQ_CHARGE = 20.0;
		const float FREQ_PROJECTILE = 3.0;
		const float FREQ_SHOCK_STORM = 30.0;
		const int DMG_BURST = 100;
		const int DMG_LUNGE = 200;
		const int DMG_STAFF = 100;
		const int DOT_FROST = 30;
		const int DOT_SHOCK = 60;
		const int ICE_CIRCLE_RAD = 172;
		const int ICE_CIRCLE_FXRAD = 172;
		const string CL_FX_SCRIPT = "monsters/elemental_ice_guardian_cl";
		const string SOUND_PROJECTILE = "magic/ice_strike.wav";
		const string SOUND_STAFF_ON = "magic/elecidle.wav";
		const string SOUND_SHOCK_LOOP = "magic/bolt_loop.wav";
		const string SOUND_SHOCK_START = "magic/bolt_start.wav";
		const string SOUND_SHOCK_HIT = "magic/bolt_end.wav";
		const string SOUND_STRUCK1 = "debris/glass1.wav";
		const string SOUND_STRUCK2 = "debris/glass2.wav";
		const string SOUND_STRUCK3 = "debris/glass3.wav";
		const string SOUND_SWIPE1 = "zombie/claw_miss1.wav";
		const string SOUND_SWIPE2 = "zombie/claw_miss2.wav";
		const string SOUND_SWIPE_STRONG1 = "zombie/claw_strike1.wav";
		const string SOUND_SWIPE_STRONG2 = "zombie/claw_strike2.wav";
		const string SOUND_SWIPE_STRONG3 = "zombie/claw_strike3.wav";
		const string SOUND_ATTACK1 = "monsters/ice_guardian/c_elemwatr_atk1.wav";
		const string SOUND_ATTACK2 = "monsters/ice_guardian/c_elemwatr_atk2.wav";
		const string SOUND_ATTACK3 = "monsters/ice_guardian/c_elemwatr_atk3.wav";
		const string SOUND_TAUNT = "monsters/ice_guardian/c_elemwatr_bat1.wav";
		const string SOUND_ALERT = "monsters/ice_guardian/c_elemwatr_slct.wav";
		const string SOUND_SUMMON = "monsters/ice_guardian/c_elemwatr_bat2.wav";
		const string SOUND_DODGE = "monsters/ice_guardian/c_elemwatr_slct.wav";
		const string SOUND_PAIN1 = "monsters/ice_guardian/c_elemwatr_hit1.wav";
		const string SOUND_PAIN2 = "monsters/ice_guardian/c_elemwatr_hit2.wav";
		const string SOUND_DEATH = "monsters/ice_guardian/c_elemwatr_dead.wav";
	}
	...
	void frame_spell()
	{
		...
		if (GUARD_ELEMENT == "ice")
		{
			...
			{
				TossProjectile("proj_freezing_sphere", /* TODO: $relpos */ $relpos(0, 16, 16), m_hAttackTarget, 100, 0, 0, "none");
			}
			else
			{
				...
				SetAngles("view.roll");
				SetAngles("view.yaw");
				TossProjectile("proj_freezing_sphere", /* TODO: $relpos */ $relpos(0, 16, 16), "none", 100, 0, 0, "none");
			}
			ANIM_ATTACK = "attack1";
			ATTACK_RANGE = ATTACK_RANGE_DEF;
			ATTACK_HITRANGE = ATTACK_HITRANGE_DEF;
	...
	void longatk_dodamage()
	{
		...
		MELEE_ATTACK = 0;
		if (!(param1)) return;
		string RND_RL = Random(-200.0, 100.0);
