...
class GuardianIron : CGameScript
{
	...
	GuardianIron()
	{
		...
		const string ANIM_ATTACK1_READY = "ready_attack1";
		const string ANIM_ATTACK2_READY = "ready_attack2";
		const string ANIM_DRAW_SWORD = "draw";
		const string ANIM_RECHARGE = "recharge";
		const string ANIM_REACH = "reach";
		...
		ATTACK_RANGE = 200;
		ATTACK_HITRANGE = 300;
		const int GUARDIAN_TYPE = 1;
		const int GUARDIAN_BEAM_SWORD = 1;
		const string DMG_ELEF_TYPE = "lightning_effect";
		const int PITCH_SWORD_OFF = 100;
		const float FREQ_CL_REFRESH = 30.0;
		GAME_PUSH_RATIO = 0.25;
		const int MAX_CHARGE_LEVEL = 50;
		const float CHARGE_DROP_RATE = 1.0;
		const string GUARDIAN_CL_SCRIPT = "monsters/guardian_iron_cl";
		const float BATTERY_LIFE = 60.0;
		const int STANCE_RANGE = 500;
		const int MAX_RECHARGE_RANGE = 400;
		const int DMG_SWING_ZAP = 700;
		const int DMG_SWING_BLADE = 400;
		const int DMG_REACH = 500;
		const int DOT_REACH = 100;
		const int DMG_SMASH = 400;
		const int SWIPE_AOE = 384;
		const int SMASH_AOE = 128;
		const string FREQ_STOMP = Random(10.0, 20.0);
		const string FREQ_REACH = Random(10.0, 20.0);
		const string FREQ_SMASH = Random(10.0, 30.0);
		const int SWORD_HILT_IDX = 0;
		const int SWORD_TIP_IDX = 1;
		const int RHAND_IDX = 2;
		const int LHAND_IDX = 3;
		const string SOUND_STEP1_START = "monsters/guardian/joint_move1.wav";
		const string SOUND_STEP2_START = "monsters/guardian/joint_move2.wav";
		const string SOUND_STEP1_LAND = "monsters/guardian/step1.wav";
		const string SOUND_STEP2_LAND = "monsters/guardian/step2.wav";
		const string SOUND_ATTACK1 = "monsters/guardian/swing1.wav";
		const string SOUND_ATTACK2 = "monsters/guardian/swing2.wav";
		const string SOUND_SWING = "weapons/swinghuge.wav";
		const string SOUND_PAIN1 = "monsters/guardian/pain1.wav";
		const string SOUND_PAIN2 = "monsters/guardian/pain2.wav";
		const string SOUND_PAIN3 = "monsters/guardian/pain3.wav";
		const string SOUND_STRUCK1 = "monsters/guardian/struck1.wav";
		const string SOUND_STRUCK2 = "monsters/guardian/struck2.wav";
		const string SOUND_RAWR = "monsters/guardian/rawr.wav";
		const string SOUND_REACH = "monsters/guardian/sca_dragelec.wav";
		const string SOUND_RECHARGE_START = "monsters/guardian/sca_dragelec.wav";
		const string SOUND_SWORD_IDLE = "magic/bolt_loop.wav";
		const string SOUND_SWORD_DRAW = "magic/elecidlepop.wav";
		const string SOUND_SWORD_OFF = "magic/elecidle.wav";
		const string SOUND_DEATH = "monsters/guardian/death.wav";
		IMMUNE_VAMPIRE = 1;
	...
	void OnHuntTarget(CBaseEntity@ target)
	{
		if ((IsEntityAlive(GetOwner())))
		{
			...
			EmitSound(GetOwner(), 0, /* TODO: $get_array */ $get_array(ROBOCOP_ARRAY, ROBOCOP_IDX), 10);
			ROBOCOP_IDX += 1;
		}
	...
	void say_robocop()
	{
		...
		ROBOCOP_NSOUNDS = /* TODO: $get_array_amt */ $get_array_amt(ROBOCOP_ARRAY);
		ROBOCOP_IDX = 0;
		ROBOCOP_NSOUNDS -= 1;
		NPC_FIGHTS_NPCS = 1;
		if ((G_DEVELOPER_MODE))
