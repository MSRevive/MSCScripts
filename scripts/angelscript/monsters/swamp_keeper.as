...
class SwampKeeper : CGameScript
{
	...
	SwampKeeper()
	{
		...
		const string SOUND_ATTACK1 = "monsters/keeper/c_troll_atk1.wav";
		const string SOUND_ATTACK2 = "monsters/keeper/c_troll_atk2.wav";
		const string SOUND_ATTACK3 = "monsters/keeper/c_troll_atk3.wav";
	...
	void my_target_died()
	{
		if (!(param1 == LAST_TARGET)) return;
		if (!(GetGameTime() > NEXT_CHEER)) return;
		NEXT_CHEER = GetGameTime();
		NEXT_CHEER += 20.0;
		PlayAnim("critical", "idle_hit_ground");
		EmitSound(GetOwner(), 0, "monsters/keeper/c_troll_bat2.wav", 10);
	}
	...
	void frame_run_step()
	{
		...
		if (RUN_STEP == 1)
		{
			EmitSound(GetOwner(), 0, "monsters/keeper/step1.wav", 5);
		}
		else
		{
			EmitSound(GetOwner(), 0, "monsters/keeper/step2.wav", 5);
			RUN_STEP = 0;
		}
	}

	void frame_throw_start()
	{
		EmitSound(GetOwner(), 0, "bullchicken/bc_attack1.wav", 10);
	}
	...
	void frame_throw1()
	{
		...
		string BOMB_SPEED = Random(400, 600);
		if (GetEntityRange(m_hAttackTarget) > 800)
		{
			string BOMB_SPEED = Random(700, 1000);
		}
		NEXT_THROW = GetGameTime();
		NEXT_THROW += 5.0;
		TossProjectile("proj_glob", /* TODO: $relpos */ $relpos(-10, 0, 32), "none", BOMB_SPEED, DMG_GLOB, 0.1, "none");
	}
