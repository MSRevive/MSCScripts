...
class Spider : CGameScript
{
	string ANIM_ATTACK;
	string ANIM_DEATH;
	string ANIM_DODGE;
	string ANIM_IDLE;
	string ANIM_LATCH_ATTACK;
	string ANIM_LATCH_HIT;
	string ANIM_LATCH_OFF;
	string ANIM_LATCH_ON;
	string ANIM_RUN;
	string ANIM_WALK;
	...
	string SOUND_LATCH_MNTR;
	string SOUND_LATCH_PLYR;
	string SOUND_PAIN;
	float SPIDER_IDLE_DELAY;
	int SPIDER_IDLE_VOL;
	int SPIDER_LATCHATTACK;
	int SPIDER_LATCHED;
	int SPIDER_LATCHING;
	int SPIDER_LATCH_ATKCHANCE;
	int SPIDER_LATCH_ATKDMG;
	int SPIDER_LATCH_ATKDUR;
	int SPIDER_LATCH_MAXRANGE;
	...
	Spider()
	{
		...
		MOVE_RANGE = 32;
		ATTACK_RANGE = 80;
		ATTACK_DAMAGE_LOW = 2.0;
		ATTACK_DAMAGE_HIGH = 3.0;
		ATTACK_ACCURACY = 0.6;
		FIN_EXP = 12;
		...
		SPIDER_VOLUME = 10;
		SPIDER_LATCHATTACK = 1;
		HUNT_AGRO = 1;
		RETALIATE_CHANCE = 0.75;
		CAN_FLEE = 0;
		SPIDER_MODEL = "monsters/spider.mdl";
		SPIDER_LATCH_ATKDMG = 5;
		SPIDER_LATCH_ATKDUR = 4;
		SPIDER_LATCH_ATKCHANCE = 20;
		SPIDER_LATCH_MAXRANGE = 200;
		ANIM_LATCH_ATTACK = "jumpmiss";
		ANIM_LATCH_HIT = "jumphit";
		ANIM_LATCH_ON = "hitbite";
		ANIM_LATCH_OFF = "falloff";
	...
	void OnRepeatTimer()
	{
		...
		if (GetEntityDist(HUNT_LASTTARGET) < SPIDER_LATCH_MAXRANGE)
		{
		}
	...
	void OnSpawn() override
	{
		...
		SetName("Leaping Cave Spider");
		NPC_GIVE_EXP = 16;
		SetModel(SPIDER_MODEL);
	...
	void spider_latch_hit()
	{
		...
		ApplyEffect(SPIDER_LATCH_TARGET, "effects/effect_spiderlatch", SPIDER_LATCH_ATKDUR, GetEntityIndex(GetOwner()), SPIDER_LATCH_ATKDMG);
		if ((IsValidPlayer(SPIDER_LATCH_TARGET)))
		{
			EmitSound(GetOwner(), "game.sound.body", SOUND_LATCH_PLYR, "game.sound.maxvol");
		}
		else
		{
			EmitSound(GetOwner(), "game.sound.body", SOUND_LATCH_MNTR, "game.sound.maxvol");
		}
