...
class HydraFireLesser : CGameScript
{
	...
	HydraFireLesser()
	{
		...
		const string ANIM_ALERT = "anim_alert";
		const string ANIM_BITE1 = "anim_bite1";
		const string ANIM_BITE2 = "anim_bite2";
		const string ANIM_BITE3 = "anim_bite3";
		const string ANIM_BREATH_RAPID = "anim_breath1";
		const string ANIM_BREATH_CONE = "anim_breath2";
		const string ANIM_BREATH_GUIDED = "anim_breath3";
		const string ANIM_HEADWHIP = "anim_whip_head";
		const string ANIM_TAILWHIP = "anim_whip_tail";
		const string ANIM_FLINCH1 = "anim_flinch1";
		const string ANIM_FLINCH2 = "anim_flinch2";
		const int ATTACH_HEAD_LEFT = 0;
		const int ATTACH_HEAD_MID = 1;
		const int ATTACH_HEAD_RIGHT = 2;
		const int ATTACH_TAIL = 3;
		const int ATTACH_HEAD_LEFT1 = 1;
		const int ATTACH_HEAD_MID1 = 2;
		const int ATTACH_HEAD_RIGHT1 = 3;
		const int ATTACH_TAIL1 = 4;
		GAME_PUSH_RATIO = 0.1;
		const int HYDRA_ATTACK_RANGE = 246;
		const int HYDRA_ATTACK_HITRANGE = 280;
		ATTACK_RANGE = HYDRA_ATTACK_RANGE;
		ATTACK_HITRANGE = HYDRA_ATTACK_HITRANGE;
		NPC_GIVE_EXP = 750;
		const string SOUND_DEATH = "monsters/hydra/death_final.wav";
		NPC_HACKED_MOVE_SPEED = 50;
		const int NPC_USE_PAIN = 1;
		const int NPC_USE_FLINCH = 1;
		const float NPC_FLINCH_THRESH = 0.01;
		const int NPC_USE_IDLE = 1;
		const string SOUND_IDLE1 = "monsters/hydra/c_bulette_slct.wav";
		const string SOUND_IDLE2 = "monsters/hydra/c_bulette_dead1.wav";
		const string SOUND_IDLE3 = "monsters/hydra/c_bulette_dead2.wav";
		const string SOUND_PAIN1 = "monsters/hydra/c_bulette_hit1.wav";
		const string SOUND_PAIN2 = "monsters/hydra/c_bulette_hit2.wav";
		const string SOUND_PAIN3 = "monsters/hydra/c_bulette_hit3.wav";
		const string HYDRA_TYPE = "fire";
		const int HYDRA_SIZE = 1;
		const int HYDRA_RUN_SPEED = 150;
		const int HYDRA_WALK_SPEED = 50;
		const string AURA_CL_SCRIPT = "monsters/hydra_fire_lesser_cl";
		const string AURA_ELEMENT = "fire_effect";
		const float FREQ_CL_REFRESH = 30.0;
		const Vector3 COL_BEAM1 = Vector3(255, 0, 0);
		const Vector3 COL_BEAM2 = Vector3(255, 128, 0);
		const int DOT_DMG = 50;
		const float DOT_DUR = 5.0;
		const string DOT_SCRIPT = "effects/dot_fire";
		const string DOT_EFFECTNAME = "DOT_fire";
		const int DOTALT_DMG = 100;
		const float DOTALT_DUR = 5.0;
		const string DOTALT_SCRIPT = "effects/dot_fire";
		const string DOTALT_EFFECTNAME = "DOT_fire";
		const int DMG_BITE1 = 150;
		const int DMG_BITE2 = 175;
		const int DMG_BITE3 = 200;
		const int DMG_TAIL = 600;
		const int AOE_TAIL = 164;
		const float FREQ_BREATH = 20.0;
		const float FREQ_TAIL_CHECK = 10.0;
		const int DMG_PROJ_RAPID = 200;
		const string PROJ_RAPID_SCRIPT = "proj_fire_bomb";
		const int DMG_BREATH_CONE = 100;
		const int BREATH_CONE_RANGE = 256;
		const int BREATH_CONE_AOE = 256;
		const int DMG_PROJ_GUIDED = 1000;
		const string PROJ_GUIDED_SCRIPT = "proj_elemental_guided";
		const string PROJ_GUIDED_ELEMENT = "fire";
		const string SOUND_COMBAT1 = "monsters/hydra/c_bulette_bat1.wav";
		const string SOUND_COMBAT2 = "monsters/hydra/c_bulette_bat2.wav";
		const string SOUND_COMBAT3 = "monsters/hydra/c_bulette_dead3.wav";
		const string SOUND_ATTACK_HEAD1 = "monsters/hydra/c_bulette_atk1.wav";
		const string SOUND_ATTACK_HEAD2 = "monsters/hydra/c_bulette_atk2.wav";
		const string SOUND_ATTACK_HEAD3 = "monsters/hydra/c_bulette_atk3.wav";
		const string SOUND_TAIL = "weapons/swing_huge.wav";
		const string SOUND_ALERT = "monsters/hydra/rawr.wav";
		const string SOUND_BREATH1_START = "monsters/hydra/c_bulette_bat1.wav";
		const string SOUND_BREATH1_STRIKE = "magic/fireball_large.wav";
		const string SOUND_BREATH2_START = "monsters/hydra/c_bulette_bat2.wav";
		const string SOUND_BREATH2_STRIKE = "magic/flame_loop_start.wav";
		const string SOUND_BREATH2_LOOP = "magic/flame_loop.wav";
