...
class SkeletonArcherBase : CGameScript
{
	...
	SkeletonArcherBase()
	{
		...
		const string ANIM_ARROW = "shootarrow";
		const string ANIM_SWIPE = "attack1";
		const string ANIM_DEATH_IDLE = "dead_on_stomach";
		const string ANIM_REBIRTH = "getup";
		const string ANIM_SIT_IDLE = "sitidle";
		const string ANIM_SIT_STAND = "sitstand";
		NPC_RANGED = 1;
		...
		DROP_GOLD = 1;
		AM_SKELETON = 1;
		const int CUSTOM_TURN_UNDEAD = 1;
		const Vector3 SKELE_ARROW_OFS = Vector3(10, 32, 68);
		const int DMG_ARROW = 5;
		const int DMG_SWIPE = 3;
		const int SWIPE_RANGE = 70;
		const int SWIPE_HITRANGE = 120;
		const int SKELE_ARROW_RANGE = 1024;
		const string SKELE_ARROW_SCRIPT = "proj_arrow_npc_dyn";
		const int SKELE_ARROW_SPEED = 800;
		...
		SKELE_COF_A = 0;
		SKELE_COF_B = 1;
		const int MIDX_SHOW_ARROW = 11;
		const int MIDX_HIDE_ARROW = 10;
		const string SOUND_STRETCH = "monsters/archer/stretch.wav";
		const string SOUND_SHOOT = "monsters/archer/bow.wav";
