...
class GuardianFireCl : CGameScript
{
	string BURST_TYPE;
	int CHARGER_ON;
	int CYCLE_ANGLE;
	...
	GuardianFireCl()
	{
		const Vector3 PASSIVE_GLOW_COLOR = Vector3(64, 16, 0);
		const int PASSIVE_GLOW_RAD = 64;
		const int SWORD_GLOW_RAD = 128;
		const Vector3 SWORD_GLOW_COLOR = Vector3(255, 96, 0);
		const int CHARGER_GLOW_RAD = 640;
		const Vector3 CHARGER_GLOW_COLOR = Vector3(255, 96, 0);
		const string CHARGER_SPRITE = "c-tele1.spr";
		const int CHARGER_SPRITE_NFRAMES = 25;
		const string SWORD_SPRITE = "fire1_fixed.spr";
		...
		const string SOUND_ZAP = "magic/sff_explsonic.wav";
	...
	void game_prerender()
	{
		...
		else
		{
			...
			ClientEffect("beam_points", BEAM_START, BEAM_END, "fire1_fixed.spr", 0.1, 5.0, 1.5, 255, 1, 30, Vector3(255, 64, 0));
		}
	}
	...
	void charger_fx_loop()
	{
		...
		ClientEffect("beam_points", BEAM_START, BEAM_END, "fire1_fixed.spr", 0.1, 10.0, 0.5, 255, 1, 30, Vector3(255, 60, 0));
		string BEAM_START = FX_CHARGER_ORG;
	...
	void grab_sprite_on()
	{
		ClientEffect("tempent", "sprite", CHARGER_SPRITE, /* TODO: $getcl */ $getcl(FX_OWNER, LHAND_INDEX), "setup_grab_sprite");
	}
	...
	void update_spit_sprite()
	{
		...
		ClientEffect("beam_points", BEAM_START, BEAM_END, "fire1_fixed.spr", 0.1, 2.0, 2.0, 255, 1, 30, Vector3(255, 60, 0));
	}
	...
	void setup_stomp_sprite()
	{
		...
		ClientEffect("tempent", "set_current_prop", "rendercolor", Vector3(255, 128, 0));
	...
	void setup_grab_targ_sprite()
	{
		...
		ClientEffect("tempent", "set_current_prop", "fadeout", "lifetime");
		ClientEffect("tempent", "set_current_prop", "rendercolor", Vector3(255, 64, 0));
	...
	void setup_grab_sprite()
	{
		...
		ClientEffect("tempent", "set_current_prop", "scale", 1.0);
		ClientEffect("tempent", "set_current_prop", "gravity", 0);
		ClientEffect("tempent", "set_current_prop", "follow", FX_OWNER, 3);
	}
