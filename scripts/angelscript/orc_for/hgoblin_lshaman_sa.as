...
class HgoblinLshamanSa : CGameScript
{
	...
	HgoblinLshamanSa()
	{
		const string ANIM_AIM = "zap_aim";
		const string ANIM_CAGE_SUSTAIN = "zap_cycle";
		...
		const float FREQ_CAGE = 30.0;
		const int DOT_CAGE = 60;
		const string FREQ_REPEL = Random(30.0, 40.0);
		ATTACK_HITCHANCE = 0.9;
		const int AOE_ZAP = 128;
		const float DUR_ZAP = 10.0;
		const float DMG_ZAP = 40.0;
		const int ATTACH_HAND = 0;
		const string CL_SCRIPT = "monsters/djinn_lightning_lesser_cl";
		const float FREQ_FX_REFRESH = 15.0;
		const string SOUND_BEAM_START = "magic/bolt_start.wav";
		const string SOUND_BEAM_LOOP = "magic/bolt_loop.wav";
		const string SOUND_CAGE_PREP = "magic/lightning_powerup.wav";
		const string SOUND_GIGGLE = "monsters/goblin/c_goblinwiz_bat1.wav";
	}
