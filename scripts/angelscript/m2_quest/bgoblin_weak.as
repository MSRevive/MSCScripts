...
class BgoblinWeak : CGameScript
{
	...
	BgoblinWeak()
	{
		...
		const string DMG_CLUB = RandomInt(8, 15);
		const string DMG_AXE = RandomInt(10, 20);
		const string DMG_SWORD = RandomInt(12, 15);
		const string GOB_TYPE = RandomInt(1, 3);
		const int GOBLIN_JUMPRANGE = 512;
		const int DMG_FIREBALL = 10;
		const int DMG_FIREBALL_DOT = 5;
	...
	{
		TOSS_FIREBALL = 0;
		TossProjectile("proj_fire_ball", /* TODO: $relpos */ $relpos(0, 48, -28), m_hAttackTarget, 400, DMG_FIREBALL, 0.5, "none");
