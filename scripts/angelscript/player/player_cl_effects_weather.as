...
class PlayerClEffectsWeather : CGameScript
{
	...
	{
		const int WEATHER_CHANNEL = 5;
		const string SOUND_RAIN = "weather/rain.wav";
		const string SOUND_RAIN_START = "weather/rain_start.wav";
		const string SOUND_DAY = "amb/birds01.wav";
		const string SOUND_NIGHT = "amb/wolf01.wav";
		const string SOUND_SNOW = "amb/wind.wav";
		const float FREQ_WEATHER_SNOW_SOUND = 7.0;
		const float FREQ_WEATHER_RAIN_SOUND = 14.6;
		Precache(SOUND_RAIN);
		const string WEATHER_SPRITE_RAIN = "rain.spr";
		const string WEATHER_SPRITE_RAIN_SPLASH = "rain_splash.spr";
		const string WEATHER_SPRITE_RAIN_MIST = "rain_mist.spr";
		const string WEATHER_SPRITE_RAIN_RIPPLE = "rain_ripple.spr";
		const string WEATHER_SPRITE_SNOW = "snow1.spr";
		const string WEATHER_SPRITE_BREATH = "char_breath.spr";
		const int WEATHER_RAIN_DROP_MAXRATE = 10;
		const int WEATHER_RAIN_MAXVOL = 5;
		const string WEATHER_RAIN_TINT_NIGHT = 0.1 + "," + 0.1 + "," + 0.1 + "," + 0.3;
		const string WEATHER_RAIN_TINT_DAY = 0.1 + "," + 0.1 + "," + 0.1 + "," + 0.3;
		const string WEATHER_RAIN_TINT_DUSK = 0.1 + "," + 0.1 + "," + 0.1 + "," + 0.3;
		const Vector3 WEATHER_RAIN_FOG_COLOR = Vector3(0.4, 0.4, 0.5);
		const float WEATHER_RAIN_FOG_DENSITY = 0.0007;
		const int WEATHER_RAIN_FOG_START = 0;
		const int WEATHER_RAIN_FOG_END = 4000;
		const int WEATHER_RAIN_FOG_ON = 1;
		const int WEATHER_RAIN_FOG_UNDERGROUND = 0;
		const int WEATHER_RAIN_RADIUS = 1024;
		const int WEATHER_STORM_DROP_MAXRATE = 20;
		const int WEATHER_STORM_MAXVOL = 10;
		const string FREQ_STORM_LIGHTNING = Random(20.0, 30.0);
		WEATHER_SNOW_DROP_RATE = 5;
		const string WEATHER_SNOW_TINT = 0 + "," + 0 + "," + 0 + "," + 0;
		const Vector3 WEATHER_SNOW_FOG_COLOR = Vector3(1, 1, 1);
		WEATHER_SNOW_FOG_DENSITY = 0.4;
		WEATHER_SNOW_FOG_START = 64;
		const int WEATHER_SNOW_FOG_END = 2048;
		const int WEATHER_SNOW_FOG_ON = 1;
		const Vector3 WEATHER_SNOW_FOG_COLOR_NIGHT = Vector3(0.8, 0.8, 0.8);
		const Vector3 WEATHER_SNOW_FOG_COLOR_DUSK = Vector3(0.9, 0.8, 0.8);
		const int WEATHER_SNOW_FOG_UNDERGROUND = 0;
		const int WEATHER_SNOW_RATE = 4;
		const string FREQ_SNOW_CHAR_BREATH = Random(10.0, 30.0);
		const int WEATHER_SNOW_RADIUS = 1024;
		const string WEATHER_TOD_DAY_TINT = 0 + "," + 0 + "," + 0 + "," + 0;
		const float WEATHER_TOD_DAY_LIGHTGAMMA = 2.5;
		const string WEATHER_TOD_DUSK_TINT = 0.8156 + "," + 0.368627 + "," + 0.007843 + "," + 0.1;
		const float WEATHER_TOD_LIGHTGAMMA = 5.0;
		const string WEATHER_TOD_NIGHT_TINT = 0.02 + "," + 0.02 + "," + 0.3 + "," + 0.1;
		const float WEATHER_TOD_NIGHT_LIGHTGAMMA = 4.0;
		const Vector3 WEATHER_TOD_NIGHT_FOG_COLOR = Vector3(0, 0, 0);
		const float WEATHER_TOD_NIGHT_FOG_DENSITY = 0.05;
		const int WEATHER_TOD_NIGHT_FOG_START = 256;
		const int WEATHER_TOD_NIGHT_FOG_END = 1024;
		TOD_STATE = "day";
		WEATHER_TYPE = "clear";
	...
	void weather_make_rain()
	{
		...
		string DROP_X_ADJ = Random(/* TODO: $neg */ $neg(WEATHER_RAIN_RADIUS), WEATHER_RAIN_RADIUS);
		string DROP_Y_ADJ = Random(/* TODO: $neg */ $neg(WEATHER_RAIN_RADIUS), WEATHER_RAIN_RADIUS);
		DROP_POS += "x";
		DROP_POS += "y";
		DROP_POS = "z";
	...
	{
		string DROP_POS = /* TODO: $getcl */ $getcl("game.localplayer.index", "origin");
		string DROP_X_ADJ = Random(/* TODO: $neg */ $neg(WEATHER_SNOW_RADIUS), WEATHER_SNOW_RADIUS);
		string DROP_Y_ADJ = Random(/* TODO: $neg */ $neg(WEATHER_SNOW_RADIUS), WEATHER_SNOW_RADIUS);
		DROP_POS += "x";
		DROP_POS += "y";
		DROP_POS = "z";
		if (!(/* TODO: $get_under_sky */ $get_under_sky(DROP_POS))) return;
