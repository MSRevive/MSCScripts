...
class ManaImmunePoison : CGameScript
{
	int ANIM_DRINK;
	int ANIM_IDLE;
	string ANIM_PREFIX;
	...
	{
		SetName("Antidote");
		SetDescription("Grantes immunity to poison , for a time.");
		SetWeight(1);
		SetSize(1);
		SetValue(500);
	...
	void game_drink()
	{
		CallExternal(GetEntityIndex(GetOwner()), "ext_register_element", "poiip", "poison", 100);
		RemoveEffect("poison");
		drink_now();
		if (!(true)) return;
