...
class ManaImmuneCold : CGameScript
{
	int DRINK_AMOUNT;
	int DRINK_EFFECTAMT;
	...
	void drink_spawn()
	{
		SetName("Cold Immunity Potion");
		SetDescription("Makes the imbiber immune to freezing effects , for a time");
		SetWeight(1);
		SetSize(1);
		SetValue(2000);
	...
	void game_drink()
	{
		CallExternal(GetEntityIndex(GetOwner()), "ext_register_element", "colip", "cold", 100);
		SendPlayerMessage(A, "magical fire of infinite warmth infuses with your soul.");
		drink_now();
		if (!(true)) return;
