...
class BaseWeaponNew : CGameScript
{
	...
	BaseWeaponNew()
	{
		...
		const int ATK1_RANGE = 90;
		const int ATK1_DMG = 120;
		const int ATK1_DMG_RANGE = 10;
		const string ATK1_DMG_TYPE = "blunt";
		const int ATK1_STAMINA = 1;
		const string ATK1_SKILL = "polearms";
		const int ATK1_ACCURACY = 85;
		...
		const string ATK1_PANIM = "pole_swing";
		const int ATK1_VANIM = 4;
		const string SOUND_ATK1 = "weapons/cbar_miss1.wav";
		const int PITCH_ATK1 = 100;
		const int ATK1_IS_PROJECTILE = 0;
		const string ATK1_PROJECTILE = "arrow";
		const int ATK1_AMMODRAIN = 1;
		const int ATK1_COF = 0;
