...
class ManaImmuneLightning : CGameScript
{
	int DRINK_AMOUNT;
	int DRINK_EFFECTAMT;
	...
	void drink_spawn()
	{
		SetName("Lodestone Juice Potion");
		SetDescription("Extracted from a mystical electrical stone");
		SetWeight(1);
		SetSize(1);
		SetValue(1000);
	...
	void game_drink()
	{
		CallExternal(GetEntityIndex(GetOwner()), "ext_register_element", "ligip", "lightning", 100);
		ClientEvent("new", "all", "effects/sfx_lightning_shield", GetEntityIndex(GetOwner()), 128, 3.0, 0);
		EmitSound(GetOwner(), 0, "magic/bolt_end.wav", 10);
		drink_now();
		if (!(true)) return;
