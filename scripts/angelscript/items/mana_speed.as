...
class ManaSpeed : CGameScript
{
	...
	void drink_spawn()
	{
		...
		SetDescription("Grants tremendous speed to the imbiber , for a time");
		SetWeight(1);
		SetSize(1);
		SetValue(1000);
	...
	void game_drink()
	{
		ClientEvent("new", "all_in_sight", "effects/sfx_motionblur", GetEntityIndex(GetOwner()), 1800);
		CallExternal(GetOwner(), "plr_change_speed", 1800, 4.0, GetEntityProperty(GetOwner(), "itemname"));
		SendPlayerMessage("You", "are filled with the Spirit of the Wolf.");
		drink_now();
		if (!(true)) return;
