...
class ManaResistFire : CGameScript
{
	int ANIM_DRINK;
	int ANIM_IDLE;
	string ANIM_PREFIX;
	...
	void drink_spawn()
	{
		SetName("Resist Fire Potion");
		SetDescription("Reduces damage caused by burning.");
		SetWeight(1);
		SetSize(1);
		SetValue(100);
	...
	void game_drink()
	{
		CallExternal(GetEntityIndex(GetOwner()), "ext_register_element", "firep", "fire", 50);
		SendPlayerMessage(A, "cooling breeze floats accross your soul.");
		drink_now();
		if (!(true)) return;
