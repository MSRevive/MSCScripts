...
class BowsTelf3 : CGameScript
{
	...
	BowsTelf3()
	{
		...
		const string DAMAGE_TYPE = "lightning_effect";
	...
	void bow_spawn()
	{
		SetName("Torkalath Lightning Bow");
		SetDescription("This enchanted bow tingles to the touch.");
		SetWeight(100);
		SetValue(1500);
	...
	{
		if (!(true)) return;
		string OWNER_SKILL = GetSkillLevel(GetOwner(), "spellcasting.lightning");
		if (!(OWNER_SKILL < 15)) return;
		SendColoredMessage(GetOwner(), "You lack the lightning affinity to activate this bow s magic.");
		CancelAttack();
	}
