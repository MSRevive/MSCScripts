...
class ManaResistCold : CGameScript
{
	...
	void drink_spawn()
	{
		SetName("Resist Cold Potion");
		SetDescription("Helps resist the cold arctic chill.");
		SetWeight(1);
		SetSize(1);
		SetValue(100);
	...
	void game_drink()
	{
		CallExternal(GetEntityIndex(GetOwner()), "ext_register_element", "coldp", "cold", 50);
		SendPlayerMessage("You", "are now more resistant to cold attacks.");
		drink_now();
		if (!(true)) return;
