...
class PhlameBird : CGameScript
{
	...
	PhlameBird()
	{
		...
		ATTACK_RANGE = 90;
		ATTACK_HITRANGE = 180;
		const int DMG_CLAW = 250;
		const int DOT_FIRE = 40;
		const string FREQ_HORROR_BOOST = Random(3.0, 6.0);
		const int DMG_SPIT = 100;
		const int DMG_BOMB = 400;
		...
		const string SOUND_FIRE_BREATH = "monsters/goblin/sps_fogfire.wav";
		NO_CIELING_IDLE = 1;
